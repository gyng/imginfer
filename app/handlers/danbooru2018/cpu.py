...
# Original: https://github.com/RF5/danbooru-pretrained/blob/70f66ac059e2deaa1e5d7e758f7c38571b9a7223/danbooru_resnet.py#L57  # noqa: E501
# Modified to support cpu initialization via map_location="cpu" on load and strict=False
...
def bn_drop_lin(n_in: int, n_out: int, bn: bool = True, p: float = 0.0, actn=None):
    ...
    if p != 0:
        layers.append(nn.Dropout(p))  # type: ignore
    layers.append(nn.Linear(n_in, n_out))  # type: ignore
    if actn is not None:
        layers.append(actn)
    return layers
...
def resnet50(pretrained=True, progress=True, top_n=6000, cpu=True, **kwargs):
    r"""
